<!-- This page has the optional area parameter and id parameter.
  Depending on how the parameters are set, the page is related to:
  1. portfolio specific area page: [id] = name of the area;
  2. single project page coming from 'all projects': [id] = id of the project;
  3. single project page coming from 'most relevant projects'/portfolio specific area page:
     [id] = project id, [area] = area name or 'most-relevant-projects'
-->
    ...
    <div v-if="pageTypeProject"
      ...
      <div class="flex lg:flex-row lg:justify-between lg:space-y-0 lg:space-x-12 flex-col space-y-4">
        <div class="flex flex-col space-y-4 md:space-y-6 justify-between">
          ...
          <!-- Group link previous/next project -->
          <div class="flex justify-between">
            <div v-if="previousProject" class="flex-1 flex flex-row justify-start">
              <NuxtLink :to="'/portfolio/' + linkPrevious"
                class=" text-color-1000 md:text-lg text-sm font-bold flex md:space-x-2 items-center hover:transform hover:scale-105 hover:text-color-700 transition duration-200">
            ...
            <div v-if="nextProject" class="flex flex-1  flex-row justify-end">
      ...
      <!-- Group link previous/next project + return back to portfolio -->
      <div class="flex md:flex-row flex-col md:justify-between space-y-4 md:space-y-0">
        <div v-if="previousProject" class="flex flex-row justify-start md:w-1/3">
          <NuxtLink :to="'/portfolio/' + previousProject.id"
            class="w-fit md:order-none order-1 flex justify-start text-color-1000 text-lg font-bold space-x-2 items-center hover:transform hover:scale-101 hover:text-color-700 transition duration-200">
...
// Check if the parameter are given correctly (if id is not a number area should not be set)
...
if (pageTypeProject && !area) { //single project from 'All projects' page
...
} else if (pageTypeProject) { //single project from portfolio specific area page
  ...
  title = ref('Venture Capital - ' + project.title)
  description = ref('Discover an innovative project backed by our venture capital firm. Explore how our strategic investment is fueling groundbreaking advancements. Learn how our expertise and support are propelling this startup towards success. Join us on this transformative journey and witness the future.')
} else { //portfolio specific area page
  ...
  // to correctly set sideDrawer menu
  let index
  indexDrawer = 2
...
//Filter projects basing on search bar emits
...
//set Meta parameter to improve SEO
useHead({
  title,
  meta: [{
    name: 'description',
